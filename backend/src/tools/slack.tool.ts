...
const sendMessageTool: MCPTool = {
    ...
    fn: async (inputs: { channelId: string; message: string; summaryId: string }) => {
        const result = await slackService.sendMessage(inputs.channelId, inputs.message, inputs.summaryId);
        return result;
    }
};

const getChannelsTool: MCPTool = {
...
const getWorkspacesTool: MCPTool = {
...
const getMessagesBySummaryTool: MCPTool = {
    ...
    fn: async (inputs: { summaryId: string }) => {
...
export const slackTools: MCPTool[] = [sendMessageTool, getChannelsTool, getWorkspacesTool, getMessagesBySummaryTool];
